...
# Exercise A and B using 3 Features
print("Classification of class 2 with classes 1 and 3 using features 1,2,4")
binary_labels_2vs13 = np.where(iris_labels == 1, 1, 0)
iris_data_f_124 = np.hstack([iris_data[:, 0:2], iris_data[:, 3].reshape(-1, 1)])
X_train_f_124, X_test_f_124, y_train_2vs13, y_test_2vs13 = train_test_split(iris_data_f_124, binary_labels_2vs13, test_size=0.2, random_state=0, stratify=binary_labels_2vs13)
K_folds = 4
...
for c in [0.5, 1, 10, 100, 1000]:
    model = svm.SVC(kernel='linear', C=c).fit(X_train_f_124, y_train_2vs13)
...
for c in [0.5, 1, 10, 100, 1000]:
    model = svm.SVC(kernel='sigmoid', C=c).fit(X_train_f_124, y_train_2vs13)
    scores = cross_val_score(model, X_train_f_124, y_train_2vs13, cv=K_folds)
    predict = model.predict(X_test_f_124)
...
for c in [0.5, 1, 10, 100, 1000]:
    model = svm.SVC(kernel='rbf', C=c).fit(X_train_f_124, y_train_2vs13)
    scores = cross_val_score(model, X_train_f_124, y_train_2vs13, cv=K_folds)
    predict = model.predict(X_test_f_124)
...
for d in [2, 4, 6]:
    for c in [0.5, 1, 10, 100, 1000]:
        model = svm.SVC(kernel='poly', C=c).fit(X_train_f_124, y_train_2vs13)
        scores = cross_val_score(model, X_train_f_124, y_train_2vs13, cv=K_folds)
        predict = model.predict(X_test_f_124)
