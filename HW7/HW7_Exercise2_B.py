...
x = PrettyTable()
models = []
x.field_names = ["Neurons in hidden layer", "Activation function", "Training Accuracy", "Validation Accuracy", "Test Accuracy"]
for activ in ["logistic", "tanh", "relu"]:
    start = time.time()
    for num_of_neurons in [2, 5, 10, 20, 50]:
        model = MLPClassifier(hidden_layer_sizes=(num_of_neurons,), solver='sgd', activation=activ,
        ...
        x.add_row([str(num_of_neurons), activ, accuracy_score(predict_train, y_train), str(scores.mean())[:5] + " ± " + str(scores.std())[0:5],
                   str(round(accuracy_score(y_test, predict), 5))])
        # print("Finished Training with ", str(num_of_neurons), " neurons in hidden layer and ", activ, " activation function")
print(x)
